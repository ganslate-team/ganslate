...
from midaGAN.nn.generators.unet import Unet
from midaGAN.nn.generators.resnet import Resnet
from midaGAN.nn.discriminators.patchgan_discriminator import PatchGAN
from midaGAN.nn.discriminators.patchgan2d_discriminator import PatchGAN2D
...
def find_model_using_name(model_name):
    ...
    for name, cls in modellib.__dict__.items():
        if name.lower() == target_model_name.lower() \
           and issubclass(cls, BaseGAN):
            model = cls

    if model is None:
        logger.error("In %s.py, there should be a subclass of BaseGAN with class name that matches %s in lowercase." % (model_filename, target_model_name))
        exit(0)
...
def build_G(conf, device):
    ...
    if name.startswith('vnet2d'):
        ...
        generator = Unet(**generator_args, norm_type=norm_type)
    elif name.startswith('resnet'):
        generator = Resnet(**generator_args, norm_type=norm_type)
    else:
